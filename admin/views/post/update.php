...
<body>
    <div class="flex justify-start items-stretch">
        ...
        <section class="w-full">
            <div class="bg-primary p-10 w-full mb-10 flex justify-between">
                <h1 class="text-white text-3xl">Chỉnh sửa bài viết</h1>
            ...
            <form action="/admin/controllers/post.php" method="post" enctype="multipart/form-data" class="w-full sm:p-5 md:p-10 lg:p-10 mx-auto" onsubmit="handleErrorUpdatePost(this,event)">
                <div class="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-10 items-stretch mb-10">
                    <div class="flex flex-col gap-5">
                        ...
                        <div class="form-control">
                            ...
                            <input type="text" data-name="tiêu đề" class="input input-bordered" name="title" value="<?php echo $post['title'] ?>">
                            <small class="text-base text-error error-message font-semibold"></small>
                        </div>
                        <!-- mô tả -->
                        <div class="form-control">
                            <label class="text-xl">Mô tả</label>
                            <input type="text" data-name="mô tả" class="input input-bordered" name="short_description" value="<?php echo $post['short_desc'] ?>">
                            <small class="text-base text-error error-message font-semibold"></small>
                        </div>
                        <!-- danh mục sản phẩm -->
                        <div class="form-control">
                            ...
                            <select name="post_category" data-name="danh mục bài viết" class="select select-bordered">
                                ...
                                <?php foreach (get_all_post_cate() as $cate) : extract($cate); ?>
                                    <option value=<?= $id ?> <?php if ($id == $post['post_cate_id']) echo "selected" ?>><?= $name ?></option>
                                <?php endforeach; ?>
                            </select>
                            <small class="text-base text-error error-message font-semibold"></small>
                        </div>
                    ...
                    <div class="flex flex-col gap-5">
                        ...
                        <div class="form-control">
                            ...
                            <textarea name="body" data-name="nội dung bài viết">
                                <?php echo $post['body'] ?>
                            </textarea>
                            <small class="text-base text-error error-message font-semibold"></small>
                        </div>
                ...
                <div>
                    ...
                    <button type="submit" name="update_post" class="btn btn-lg hover:btn-primary w-auto">Cập nhật bài viết</button>
                </div>
    ...
    <script>
        const handleErrorUpdatePost = (form, event) => {
